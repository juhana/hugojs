...
int stack_depth;
int tail_recursion = 0;
long tail_recursion_addr = 0;
...
void RunGame(void)
{
    ...
    if( autosave_filename[0] != 0 ) {
        hugo_restore_autosave(autosave_filename);
        goto FreshInput;
    }
#endif
...
/* (All the debugger range-checking is important because invalid memory
   writes due to invalid object location calculations are a good way to
   crash the system.)
*/
...
/* Note that under Glk, stdio f...() functions map to Glk functions */
	...
	*/
	...
	switch (GetValue())
	{
		...
		case 51:		/* SYSTEM_TIME */
		{
			...
			time(&t);
			lt = localtime(&t);
			strftime(parseerr, 20, "%Y-%m-%d %H:%M:%S", lt);
#else
...
void SaveWindowData(struct SAVED_WINDOW_DATA *spw)
{
